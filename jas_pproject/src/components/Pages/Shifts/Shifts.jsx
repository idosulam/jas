...
function Shifts() {
  ...
  const deletePay = deleteTarget ? calcPay(deleteTarget.place, deleteTarget.hours) : 0;
  ...
  return (
    <section className="shifts page">
      ...
      <div className="shifts__place-filter animate-in animate-in--2" role="group" aria-label="Filter by place">
      ...
      <div className="shifts__summary animate-in animate-in--3" key={`${month}-${year}-${placeFilter}`}>
      ...
      {error && (
        <p className="shifts__error shifts__error--shake" role="alert">{error}</p>
      )}
      ...
      <div className="shifts__list-header animate-in animate-in--4">
        <h2 className="shifts__list-title">
          {MONTHS[month]} {year}
          {placeFilter !== 'all' && (
            <span className="shifts__list-subtitle"> · {PLACES[placeFilter]?.label}</span>
          )}
        </h2>
        <button type="button" className="shifts__add-btn" onClick={openAddModal}>
          + Add shift
        </button>
      </div>
      ...
      {loading ? (
        ...
        <p className="shifts__empty shifts__empty--fade" key={`empty-${placeFilter}`}>
          {placeFilter === 'all'
          ...
          >
            ...
            <form className="shifts__form" onSubmit={handleSubmit}>
                ...
                <p className="shifts__preview shifts__preview--pop">
                  ...
                  {form.tips && (
                    <> + tips {formatMoney(parseFloat(form.tips) || 0)} ={' '}
                      <strong>{formatMoney(previewPay + (parseFloat(form.tips) || 0))}</strong>
                    </>
                  )}
                </p>
              )}
              ...
              <div className="shifts__form-actions">
                <button type="button" className="shifts__btn shifts__btn--ghost" onClick={closeFormModal}>
                  Cancel
                </button>
                <button type="submit" className="shifts__btn shifts__btn--primary" disabled={saving}>
      ...
      {deleteTarget && (
        ...
        >
          ...
          >
            ...
            <h2 id="delete-modal-title" className="shifts__modal-title shifts__modal-title--delete">
              Delete this shift?
            </h2>
            ...
            <div className="shifts__delete-preview">
              <span className={`shifts__badge shifts__badge--${deleteTarget.place}`}>
                {deletePlaceInfo?.label}
              </span>
              <span className="shifts__delete-date">{deleteTarget.shift_date}</span>
